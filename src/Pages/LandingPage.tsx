...
const LandingPage = () => {
  ...
  const { isLoading: isSchoolNameLoading, schoolName } =
    ...
    const {notifications,isLoading:isNotificationsLoading,totalPages} = useNotificationsByGrade(loggedInUser?.gradeId!,page,NOTIFICATIONS_PER_PAGE);
  const [isViewNotifications,setIsViewNotifications] = useState<boolean>(false);
  ...
  return (
    <div className="min-h-screen bg-[#EEF6FF] flex flex-col items-center justify-center p-2">
      <div className="w-full max-w-md flex flex-col items-center justify-around">
        ...
        {!isNotificationsLoading && notifications.length!==0 &&  <div className="flex items-center gap-2 my-2">
          <span className="text-blue-400 font-semibold text-md">New Content</span>
          <Button onClick={() => setIsViewNotifications(true)} className="bg-blue-500 text-white hover:bg-blue-600 hover:duration-300">View</Button>
        </div>}
        {/* Description */}
        ...
        <DialogContent>
          ...
          <div className="flex flex-col gap-2">
            {notifications.map((notification,index) => {
              return <div key={notification.id} className={`flex items-center justify-between text-gray-600 font-semibold border-b p-1 ${index!==notifications.length-1 ? "border-b" : ""}`}>
                <div className="flex flex-wrap w-3/5">{notification.message}</div>
                <div>{formatDate(notification.createdAt,"d/M/y")}</div>
              </div>
            })}
          </div>
          {totalPages > 1 && <Pagination currentPage={page} noOfPages={totalPages} setCurrentPage={setPage}/>}
          <DialogFooter>
            <DialogClose asChild>
