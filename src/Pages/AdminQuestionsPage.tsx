...
const AdminQuestionsPage = () => {
  ...
  const [filterByQuestionType, setFilterByQuestionType] =
    useState<string>("ALL");
  ...
  const [
    isAddMcqAnswerModalOpenForQuestion,
    setIsAddMcqAnswerModalOpenForQuestion,
  ] = useState<{ question: QuestionType | null; isOpen: boolean }>({
    question: null,
    isOpen: false,
  });
  ...
  const {
    ...
    filterByDifficulty === "ALL"
      ? undefined
      : (filterByDifficulty as QuestionDifficulty),
    filterByQuestionType === "ALL"
      ? undefined
      : (filterByQuestionType as QuestionTypeType),
  ...
  const handleAddMcqAnswers = async () => {
    ...
    try {
      ...
      await Promise.all(
        mcqAnswers.map(async (mcqAnswer) => {
          await axios.post<{
            success: boolean;
            answer: {
              id: string;
  ...
  const isMcqAnswersValid = () => {
    ...
    const correctMcqOptions = mcqAnswers.filter(
  ...
  const handleMcqCorrectAnswerChange = (index: number) => {
    ...
    const correctAnswerIndex = mcqAnswers.findIndex(
      (answer) => answer.isCorrect === true
    );
  ...
  const handleAddMcqAnswer = () => {
    ...
    if (mcqAnswerCorrect) {
      const correctAnswerIndex = mcqAnswers.findIndex(
        (mcqAnswer) => mcqAnswer.isCorrect === true
      );
  ...
  useEffect(
    () => setPage(1),
    [searchByTitle, filterByDifficulty, filterByQuestionType, filterByStatus]
  );
    ...
    <>
      <div className="flex flex-col items-center w-full bg-[#ecfbff] min-h-screen">
        <div className="flex items-center justify-start w-full px-8 mt-14">
          <Link
            className="flex items-center gap-2 text-blue-500 font-semibold hover:text-blue-600 hover:duration-300"
            to={`/${role}/levels/${level.subjectId}`}
          >
            <ArrowLeft />
            Back to levels
          </Link>
        ...
        <div className="flex items-center justify-center w-full px-8">
          <div className="flex flex-col items-center justify-center gap-2">
            <Input
              ...
              value={searchByTitleText}
              onChange={(e) => setSearchByTitleText(e.target.value)}
            />
        ...
        <div className="flex items-center w-full py-4 px-8 rounded-lg mx-8 flex-wrap gap-2">
          <div className="bg-white">
            <Select
              ...
              onValueChange={setFilterByDifficulty}
            >
              <SelectTrigger>
          ...
          <div className="bg-white">
            <Select
              value={filterByQuestionType}
              onValueChange={setFilterByQuestionType}
            >
              <SelectTrigger>
        ...
        >
          <DialogContent>
            ...
            <div className="flex flex-col items-center justify-center w-full gap-4">
                  ...
                  <SelectContent>
                    {QUESTION_TYPES.map((questionType) => {
                      return (
                        <SelectItem
                          disabled={
                            !AVAILABLE_QUESTION_TYPES.includes(questionType)
                          }
                          key={questionType}
                          value={questionType}
                        >
              ...
              <div className="flex flex-col items-start w-full justify-center gap-2">
                <Label htmlFor="questionExplanation">
            ...
            <DialogFooter>
              <DialogClose asChild>
                <Button disabled={isAddingQuestion} variant={"outline"}>
                  Cancel
                </Button>
              </DialogClose>
              <Button
                disabled={isAddQuestionButtonDisabled() || isAddingQuestion}
        ...
        <Dialog
          open={isAddMcqAnswerModalOpenForQuestion.isOpen}
          onOpenChange={handleAddAnswerOpenChange}
        >
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription asChild>
                <div className="flex flex-col items-start justify-center w-full gap-2">
                  <div className="flex flex-wrap">
                    Question Title:{" "}
                    {isAddMcqAnswerModalOpenForQuestion.question?.questionTitle}
                  </div>
                  <div className="flex flex-wrap text-sm text-gray-700">
                    Explanation:
                    {isAddMcqAnswerModalOpenForQuestion.question?.explanation}
                  </div>
                  <div className="text-gray-700 font-semibold">
                    Difficulty:{" "}
                    {isAddMcqAnswerModalOpenForQuestion.question?.difficulty}
                  </div>
                </div>
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="flex flex-col items-start justify-center w-full gap-2">
              {mcqAnswers.map((answer, index) => {
                return (
                  <div
                    key={index}
                    className="flex items-center justify-between w-full gap-2 p-4 border rounded-lg"
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <TrashIcon
                        onClick={() => handleMcqAnswerDelete(index)}
                        className="text-red-500 cursor-pointer"
                      />
                      <Checkbox
                        checked={answer.isCorrect}
                        onCheckedChange={() =>
                          handleMcqCorrectAnswerChange(index)
                        }
                      />
                    </div>
                  </div>
            ...
            {mcqAnswers.length < 4 && (
              <div className="flex flex-col justify-center w-full gap-2">
                <Input
                  ...
                  onChange={(e) => setMcqAnswerValue(e.target.value)}
                  type="text"
                  placeholder="Add Answer"
                />
                <div className="flex items-center justify-start gap-2">
                  <Checkbox
                    checked={mcqAnswerCorrect}
                    onCheckedChange={() =>
                      setMcqAnswerCorrect(!mcqAnswerCorrect)
                    }
                  />
                  <div>Correct Answer</div>
                </div>
                <div className="flex items-center w-full justify-end">
                  <Button variant={"outline"} onClick={handleAddMcqAnswer}>
                    Add Answer
                  </Button>
                </div>
              </div>
            ...
            <DialogFooter>
              <DialogClose asChild>
                <Button disabled={isAddingMcqAnswers} variant={"outline"}>
                  Cancel
                </Button>
              </DialogClose>
              <Button
                onClick={handleAddMcqAnswers}
                disabled={!isMcqAnswersValid() || isAddingMcqAnswers}
              >
                Add
              </Button>
            </DialogFooter>
